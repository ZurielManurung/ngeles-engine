...
export default function CameraCapture({ onImageCaptured, imagePreview }: CameraCaptureProps) {
  ...
  const setVideoRef = useCallback((node: HTMLVideoElement | null) => {
    ...
    if (node && streamRef.current && node.srcObject !== streamRef.current) {
      ...
      node.play().catch((err) => {
        // Ignore harmless Chrome AbortError when play is interrupted by re-render
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      // Set camera active to mount the video tag
      setIsCameraActive(true);

      // If video element already mounted, attach stream
  ...
  return (
    <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-4 sm:p-5 flex flex-col gap-4 shadow-xl">
      ...
      {/* Main View Area */}
      <div className="relative aspect-video w-full rounded-xl overflow-hidden bg-zinc-950 border border-zinc-800 flex items-center justify-center group">
        ...
        {!isCameraActive && imagePreview && (
          <div className="relative w-full h-full">
            ...
            />
            <div className="absolute inset-0 bg-black/30 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
              <button
                type="button"
                onClick={startCamera}
          ...
          <div className="p-6 text-center flex flex-col items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1 max-w-xs">
                AI akan menganalisis raut wajah (muka bantal/panik) & kondisi latar belakang untuk alibi otentik.
              </p>
            </div>
